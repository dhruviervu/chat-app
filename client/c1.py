...
(async () => {{
  ...
  ws.onmessage = async evt => {{
    ...
    if (obj.type === "passphrase") {{
      ...
      if (currentPeer) {{
        // Auto-render current chat now that key is available
        try {{ await decryptAndDisplayChatHistory(currentPeer); }} catch(_) {{}}
      }}
      return;
    }}
    ...
    if (obj.type === "register_ok") {{
      ...
      if (!defaultPassphrase && obj.passphrase) {{
        defaultPassphrase = obj.passphrase;
        appendMsg("[Received encryption key from server]");
  ...
  // Periodically request passphrase until received
...
html = (
    html_template
        ...
        .replace("__ANON__", json.dumps(bool(anonymous)))
        .replace("__ANON_LABEL__", json.dumps(anon_label))
        .replace("__WS_BASE__", json.dumps(ws_base_url))
)
# Convert doubled braces to single braces for valid JS now that we're not using an f-string
